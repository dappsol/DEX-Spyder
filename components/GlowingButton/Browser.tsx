...
export default function Browser(props: Props) {
  ...
  useEffect(() => {
    if (refs.current.length > 0) {
      const activeRef = refs.current[props.activeTab - 1];
      ...
      props.onActiveTabChange(props.activeTab)
    }
  }, [props.activeTab]);
  return (
    <Wrapper m={props.m}>
      <BrowserPanel>
        <Navigation>
          ...
          <Tabs ref={TabsRef}>
            <Item ref={el => el && refs.current.push(el)} active={props.activeTab === 1} onClick={() => props.onActiveTabChange(1)}>Setting</Item>
            <Item ref={el => el && refs.current.push(el)} active={props.activeTab === 2} onClick={() => props.onActiveTabChange(2)}>Dashboard</Item>
          </Tabs>
...
const BrowserPanel = styled.div`
  ...
  &:before {
    ...
    top: -1px;
    left: -1px;
    mask: url("data:image/svg+xml,%3Csvg width='901' height='452' viewBox='0 0 402 202' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Crect x='0.5' y='0.5' width='401' height='201' rx='9.5' stroke='black'/%3E%3C/svg%3E%0A");
    mask-mode: alpha;
    pointer-events: none;
  }
